...
def _collect_layer_statistics(mod, data, collector, max_num_batch=None, logger=None):
    num_batches = 0
    for ib, batch in enumerate(data):
        data_ = gluon.utils.split_and_load(batch[0], ctx_list=[mx.cpu()], batch_axis=0, even_split=False)
...
def _collect_layer_output_min_max(mod, data, include_layer=None,
                                  max_num_batch=None, logger=None):
    ...
    num_batches = _collect_layer_statistics(mod, data, collector, max_num_batch, logger)
    return collector.min_max_dict, num_batches


def _collect_layer_outputs(mod, data, include_layer=None, max_num_batch=None, logger=None):
...
def my_quantize_model(sym, arg_params, aux_params, data_shape,
    ...
    if calib_mode is not None and calib_mode != 'none':
        ...
        mod = Module(symbol=sym, label_names=[], context=ctx)
        mod.bind(data_shapes=[("data", data_shape)], inputs_need_grad=False, for_training=False)
