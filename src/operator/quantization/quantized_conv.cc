...
bool QuantizedConvShape(const nnvm::NodeAttrs& attrs,
  ...
  if (param.layout.has_value()) {
    ...
    CHECK(param.layout.value() == mshadow::kNCHW || param.layout.value() == mshadow::kNCDHW)
          << "mkldnn quantized_conv only supports NCHW or NCDHW for now";
#else
  ...
  if (data_ndims == 3) {
    ...
    mxnet::TShape oshape{1, 1, 1};
    oshape[N] = dshape[N];
    oshape[C] = wshape[N];

    const index_t dilated_ksize_x = param.DilatedKernelSize(0);
    oshape[H] = (AddPad(dshape[H], param.pad[0]) - dilated_ksize_x) / param.stride[0] + 1;
