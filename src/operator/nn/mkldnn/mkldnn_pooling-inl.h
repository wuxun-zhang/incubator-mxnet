...
inline bool SupportMKLDNNPooling(const PoolingParam &param) {
  return (param.kernel.ndim() == 2 || param.kernel.ndim() == 3) &&
         (param.pool_type == pool_enum::kMaxPooling ||
          param.pool_type == pool_enum::kAvgPooling) &&
         (!param.layout.has_value() ||
         (param.layout.value() == mshadow::kNCHW || param.layout.value() == mshadow::kNCDHW));
}
...
inline bool SupportMKLDNNPooling(const PoolingParam &param,
  ...
  if (param.pooling_convention == pool_enum::kValid) {
    ...
    if (param.pool_type == pool_enum::kAvgPooling) {
      CHECK(dshape.ndim() == 4 || dshape.ndim() == 5);
      // mkldnn works differently when padding is asymmetric, so let's skip this case.
      bool is_symmetric = true &&
       (param.pad[0] == GetPaddingSizeFull(dshape[2], param.pad[0], param.pad[0], param.kernel[0],
                                           param.stride[0])) &&
       (param.pad[1] == GetPaddingSizeFull(dshape[3], param.pad[1], param.pad[1], param.kernel[1],
                                           param.stride[1])) &&
       (dshape.ndim() == 5 &&
        param.pad[1] == GetPaddingSizeFull(dshape[4], param.pad[2], param.pad[2], param.kernel[2],
                                           param.stride[2]));
      if (is_symmetric) {
        return true;
      }
